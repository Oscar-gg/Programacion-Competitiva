...
void update(int index, int uL, int uR, int val, int l, int r, vector<long long int> &seg)
{
    if (uL == l && uR == r)
    {
        seg[index] += val;
    ...
    if (uL <= mid)
    {
        update(index * 2, uL, min(mid, uR), val, l, mid, seg);
    }

    if (uR >= mid + 1)
    {
        update(index * 2 + 1, max(mid + 1, uL), uR, val, mid + 1, r, seg);
    }
}
...
void solve()
{
    int n, q;
    cin >> n >> q;
    vector<long long int> a(n), seg(4 * n, 0);
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        else
        {
            ...
            k--;
            // c--;
            cout << a[k] + query(1, 0, n - 1, k, k, seg) << "\n";
        }
    }
}
