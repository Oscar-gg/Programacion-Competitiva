...
void countPrimes(int n, vector<int> &primesC, int mul)
{
    int currI = 0;
    while (n > 1)
    {
        while (n % primes[currI] == 0 && n > 1)
        {
            primesC[currI] += mul;
            n /= primes[currI];
        }
...
void solve()
{
    ...
    vector<int> primesC(primes.size());

    for (auto x : umap)
    {
        countPrimes(x.first, primesC, x.second);
    }

    for (int i = 0; i < primesC.size(); i++)
    {
        if (primesC[i] != 0 && primesC[i] % n != 0)
        {
            cout << "NO\n";
            return;
        }
