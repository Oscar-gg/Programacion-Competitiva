...
{
    vector<vector<char>> ret = mat;

    for (int i = 0; i < ret.size(); i++)
    {
        for (int j = i; j < ret[i].size(); j++)
        {
...
// void pM(vector<vector<char>> mat){
...
void solve()
{
    ...
    vector<vector<char>> prev = mat;
    do
    {
        // pM(prev);
        vector<vector<char>> rot = rotate(prev);
        unchanged = true;
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < n; j++)
            {
                if (rot[i][j] != prev[i][j])
                {
                    ...
                    rot[i][j] = max(op1, op2) + 'a';
                    prev[i][j] = max(op1, op2) + 'a';
                }
            }
        }
        prev = rot;
