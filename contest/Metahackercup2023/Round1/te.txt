...
void explore(int remaining, int actual, vector<int> &values, int currentSum, vector<vector<int>> &answers)
{
    if (values.size() > 100 || currentSum > 41 || actual == 1)
        return;

    if (remaining == 1 && currentSum <= 41)
    {
    ...
    for (int i = actual; i >= 1; i--)
    {
        ...
        {
            values.push_back(i);
            explore(remaining / i, i, values, currentSum + i, answers);
            values.pop_back();
        }
    }
}
